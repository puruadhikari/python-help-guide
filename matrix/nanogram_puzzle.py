...
Specifically, each cell can be either black or white, which we will represent
...
For example, the instructions for one row of [ 2, 1 ] indicate that there must be a run of two black
cells, followed later by another run of one black cell, and the rest of the row filled with white
cells.
...
Given a 2D matrix representing a player's solution; and instructions for each row along with
additional instructions for each column; return True or False according to whether both sets of
instructions match.
...
def extract_run(line):
    ...
    return result
"""
NOTE : Tranpose the matrix and return False if any value does not match
"""
for items in matrix1:
    print(extract_run(items))